...
> 이 워크플로는 **기본 브랜치**(`main`)에서 스케줄로 실행되고, 같은 브랜치로 푸시합니다. 워크플로와 스크립트는 브랜치명을 하드코딩하지 않으므로(체크아웃은 트리거된 ref, 푸시는 `HEAD`) 기본 브랜치를 바꿔도 그대로 동작합니다. 다만 **워크플로 파일이 기본 브랜치에 존재해야** 스케줄이 잡힙니다.
...
> "이 레포에서 `automation/weekly-paper-update.md`의 절차대로, 지난 한 주의 GraphRAG 최신 논문을 찾아 새 OKF 노트를 추가하고 인덱스·로그를 갱신한 뒤 `main`에 커밋·푸시해줘."
